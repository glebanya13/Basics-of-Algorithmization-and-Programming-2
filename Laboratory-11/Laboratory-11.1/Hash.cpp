...
int Multiplicative(int key, int size, int p) // f хешировани€
{
	int A;
	double fr;
	A = (sqrt(5) - 1) / 2;
	int key2 = size * (modf(key * A, &fr));
	return (key2 + p) % size;
}
//-------------------------------
...
//-------------------------------
bool Object::insert(void* d) // добавление
{
	bool b = false;
	if (N != size)
...
//-------------------------------
int Object::searchInd(int key) // поиск по индексу
{
	int t = -1;
	bool b = false;
	if (N != 0)
