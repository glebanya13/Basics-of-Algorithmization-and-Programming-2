...
// Функция для поиска суммы положительных двузначных элементов
int sumOfNegativeTwoDigitElements() {
    bool IsExist = false;
    int sum = 0;
    Node* current = head;
    while (current != nullptr) {
        if (current->data > -99 && current->data < -9) {
            sum += current->data;
            IsExist = true;
        }
        current = current->next;
...
int main() {
    ...
    do {
        ...
        cout << "7. Find sum of negative two-digit elements" << endl;
        cout << "8. Exit" << endl;
        ...
        case '7':
            cout << "Sum of negative two-digit elements: " << sumOfNegativeTwoDigitElements() << endl;
